...
A \(j\)-chain in some category \(\A\) is a functor \(C : j \to \A \)\marginnote{\(j\)-chain} and we say that \(\A\) has colimits of chains if for all \(j \in \Ord\) every \(j\)-chain has a colimit.
...
When \(j\) is a limit ordinal we can calculate colimits and factorization arrows on a cofinal subchain of \(j\) rather than on

\begin{lemma}
  Let \(j\) be a limit ordinal and \(C : j \to \A\) a \(j\)-chain into a category \(\A\) with colimits of chains.
  We denote the elements of the chain by \(C_i\), for \(i \in j\), and its arrows by \(c_{i,i'} : C_i \to C_{i'}\) for \(i \leq i' < j\).
...
\begin{proof}
  ...
  Indeed by cofinality let \(i' \in j'\) be such that \(i \leq i'\); by the cocone condition we must set \(\alpha_i = \alpha_{i'} \circ c_{i,i'}\) so \((A, \alpha_{i}:C_i \to A)_{i \in j'}\) can be extended to the whole chain in only one way.

  Let \((L, \sigma_i : C_i \to L)_{i \in j}\) be the colimit of \(C\); then \((L, \sigma_i : C_i \to L)_{i \in j'}\) is the colimit of \(C'\).
  Let \((A, \alpha_i : C_i \to A)_{i \in j'}\) be a cocone on \(C'\) and then we extend it to a cocone on \(C\) as explained above.
  Now, since \(L\) is the colimit, let \(m \colon L \to A\) be the unique factorization arrow i.e. we have
  \begin{equation*}
