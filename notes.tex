...
\markboth{}{}
One of the most celebrated aspects of Category Theory is its strength as a generalization tool as well as its ability to single out similarities between different mathematical phenomena, allowing us to group them under the umbrella of more general concepts. 
This thesis testifies for both: we develop the theory of \(F\)-algebras and \(F\)-coalgebras as a generalization of Order Theory and we show how different kinds of induction (on the natural numbers and on trees, for instance) and coinduction descend from initial algebras and terminal coalgebras.
Let \((P, \sqsubseteq)\) be a partially ordered set and \(f: P \to P\) a monotone function.
A pre-fixed point of \(f\) is an element \(x\) of \(P\) such that \(f(x) \sqsubseteq x\).
It is known (and not hard to prove) that the following order theoretic lemma holds.
...
A second version of the lemma exists.
...
Beginning from the observation that partially ordered sets can be thought of as particular categories one can generalize \(P\) to a category \(\A\) and \(f\) to an endofunctor \(F: \A \to \A\).
In this way we obtain the theory of \(F\)-algebras (from generalization of pre-fixed points) and \(F\)-coalgebras (from generalization of post-fixed points); the elements \(\overline{a}\) and \(\overline{b}\) of the above lemmas become, respectively, the initial algebra of \(F\) and the terminal coalgebra of \(F\).
...
Many examples are presented (with particular attention to the case when \(F\) is a polynbomial functor) and {initial algebras} are defined.
...
\begin{definition}
  ...
  Given a regular cardinal \(\lambda\) we say that a functor \(F\) over \(\Set\) is \textbf{\(\lambda\)-accessible} \marginnote{\(\lambda\)-accessible functor} if for every set \(X\) and \(x \in FX\) there is a subset \(m : M \rightarrowtail X, |M| < \lambda\) such that \(x \in \im (Fm)\).
\end{definition}
...
\begin{example}
  ...
  Functors that are \(\aleph_0\)-accessible are also called \textbf{finitary set functors}\marginnote{finitary set functor}.
...
\begin{remark}
  \label{rem:gch}
  Recall that the \textbf{Generalized Continuum Hypothesis (GCH)}\marginnote{GCH} is the statement that, for every infinite cardinal \(\lambda\) one has \(\lambda^+ = 2^\lambda\); here with \(2^\lambda\) we indicate the power-set of \(\lambda\).
  The case when \(\lambda = \aleph_0\) is referred to simply as the \textbf{Continuum Hypothesis (CH)}\marginnote{CH}.
