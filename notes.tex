...
\begin{example}
  ...
  \(FX = X + 1\) has terminal coalgebra \(\mathbb{N}^\top\) with the following structure
  \begin{center}
    \begin{tikzcd}[sep=large]
...
\begin{remark}
  Every coalgebra \(\alpha : A \to FA\) induces a canonical cone \((\alpha_n : A \to F^n1)_{n < \omega}\) over (\ref{dia:terminal_chain}) defined by the following
  \begin{align*}
    \alpha_0 &=\ ! \\
...
Adámek's Theorem \ref{teo:adamek} dualizes straightforwardly.
...
Again this result can be used directly to prove the existence of a terminal coalgebra or [BLABLA]
