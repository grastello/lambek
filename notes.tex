...
However we'd like to investigate some sufficient conditions for the initial-algebra chain to converge.
Our investigations will eventually lead some conditions on the category \(\A\) and the functor \(F\) so that convergence of the initial-algebra chain and existence of the initial algebra are equivalent.

In order to proceed we need to recall what subobjects are.
...
\end{definition}
We can restrict ourselves to a class \(\mathcal{M}\) of monomorphism and speak of \(\mathcal{M}\)-subobjects i.e. subobjects represented by mono in \(\mathcal{M}\).
...
\begin{definition}
  ...
  We say that an object \(A\) has \textbf{smooth} \(\mathcal{M}\)\textbf{-subobjects} if \(\Sub_\mathcal{M}(A, \leq)\) \marginnote{smooth\\ \(\mathcal{M}\)-subobjects} is a chain-complete poset (particularly it is a set and not a proper class) where joins of chains are given by colimits of the corresponding chain of subobjects.
...
\begin{remark}
  ...
  To be more precise consider an ordinal \(\lambda\) and \((m_i : A_i \rightarrowtail A)_{i < \lambda}\) be a chain of \(\mathcal{M}\)-subobjects i.e. we have arrows \(a_{i,j} : A_i \to A_j\) (that need not be in \(\mathcal{M}\)) that withness \(m_i \leq m_j\) for \(i \leq j < \lambda\).
  ...
  We thus have a cocone \((c_i : A_i \to C)_{i < \lambda}\) over our chain that, for \(A\) to have smooth \(\mathcal{M}\)-subobjects, must be a colimit in \(\A\).
...
\begin{definition}
  Given \marginnote{\(\mathcal{M}\)-preserving functor} a category and a class \(\mathcal{M}\) of monomorphisms we say that an endofunctor \(F\) is \textbf{\(\mathcal{M}\)-preserving}  if for every \(m \in \mathcal{M}\) we have \(Fm \in \mathcal{M}\).
\end{definition}
...
\begin{proof}
  ...
  Finally we know that \(\alpha_i = \alpha_{i+1} \circ w_{i, i+1}\) and that, as \(\alpha_i\) and \(\alpha_{i+1}\) represent the same subobject, there is an arrow \(v : W_{i+1} \to W_i\) such that \(\alpha_{i+1} = \alpha_i \circ v\) (witnessing \(\alpha_{i+1} \leq \alpha_i\)).
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  Now from the following calculations and \(\alpha_i, \alpha_{i+1}\) being monomorphisms we obtain that \(v\) is the inverse of \(w_{i, i+1}\) and thus the initial-algebra chain converges.
  \begin{equation*}
    \alpha_i \circ v \circ w_{i, i+1} = \alpha_{i+1} \circ w_{i, i+1} = \alpha_i \implies \id_{W_i} = v \circ w_{i,i+1}
  \end{equation*}
  \begin{equation*}
    \alpha_{i+1} \circ w_{i, i+1} \circ v = \alpha_i \circ v = \alpha_{i+1} \implies \id_{W_{i+1}} = w_{i, i+1} \circ v
  \end{equation*}
\end{proof}
...
\begin{example}
  ...
  Then \(\Alg F\) does not have smooth monomorphisms as there are \(F\)-algebras, such as \((\mathbb{Z}/n\mathbb{Z}, - + 1, 0)\), that do not have \(\N\) as subobject.
  Thus the hypothesis of Theorem \ref{teo:M-pre-fixed} do not hold and indeed we can find a functor on \(\Alg F\) that has no initial algebra while having a fixed point and preserving monomorphisms.

  Denote by \(\mathcal{P}_o(X)\) the power set of \(X\) without the empty set \(\emptyset\), this is a functor on \(\Set\) with the natural blabla on arrows.
  This construction gives a functor on \(\Alg F\) that sends an algebra \((A, f, c)\) to \((\mathcal{P}_0(A), \mathcal{P}_0f, \{c\})\).
  Now \(\mathcal{P}_o\) preserves monomorphisms and the terminal algebra \(\{*\}\) is a fixed point and, up to isomorphism, the only one (CHECK).
  However \(\{*\}\) is not initial as given any algebra \((A, f, c)\) such that \(f(c) \neq c\) there can be no \(\mathcal{P}_o\)-algebra momorphisms from \(\{*\}\) to \(A\) since there is no \(F\)-algebra morphism from \(\{*\}\) to \(A\).
  \marginnote{check notation and use of parenthesis}
\end{example}

\begin{example}
  Consider the category \(\Set \times \Set\) that is well-powered and has smooth monomorphism (WHY?).
  As functor take
  \begin{equation}
    F(X, Y) = \begin{cases}
      (\emptyset, 1) & \text{if } X \neq \emptyset \\
  ...
  \(F\) does not preserve monomorphisms (WHY?) but has pre-fixed points such as \(F(1, 1) = (\emptyset, 1) \rightarrowtail (1, 1)\).
...
Finally we can put all our work so far into a single theorem, the Initial Algebra Theorem which is, among other things a converse to Lambeck's Lemma.
...
\begin{theorem}
  ...
  Then the following consitions are equivalent:
  \begin{itemize}
