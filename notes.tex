\documentclass[a4, 11pt, twoside]{memoir}
...
\markboth{}{}
One of the main features of Category Theory is its strength as a generalization tool as well as its effectiveness in identifying similarities between different mathematical phenomena, allowing us to group them under the umbrella of more general concepts. 
The theory of \(F\)-algebras and \(F\)-coalgebras was developed, starting from the 1970's, as a categorical generalization of Order Theory and allows for different kinds of induction (on the natural numbers and on trees, for instance) and coinduction to be recognized as descending from initial algebras and terminal coalgebras.
To illustrate this let \((P, \sqsubseteq)\) be a partially ordered set and \(f: P \to P\) a monotone function.
A pre-fixed point of \(f\) is an element \(x\) of \(P\) such that \(f(x) \sqsubseteq x\).
It is known (see Lemma \ref{lemma:order_theory}) that the following order theoretic lemma holds.

\begin{quote}
  \textbf{Lemma 1.}
  If \(A\) is the set of all pre-fixed points of \(f\) and \(A\) has a meet \(\overline{a}\) then \(\overline{a}\) is the least fixed point of \(f\).
\end{quote}

A second, dual, version of the lemma exists (see Lemma \ref{lemma:order_theory_ii}).

\begin{quote}
  \textbf{Lemma 2.}
  If \(B\) is the set of all post-fixed points of \(f\) (i.e. all elements \(x \in P\) such that \(x \sqsubseteq f(x)\)) and \(B\) has a join \(\overline{b}\) then \(\overline{b}\) is the biggest fixed point of \(f\).
\end{quote}

From any of the above lemmas follows the celebrated Knaster-Tarski Fixed Point Theorem \ref{teo:knaster_tarski}.
Starting from the observation that partially ordered sets can be thought of as particular categories one can generalize \(P\) to a category \(\A\) and \(f\) to an endofunctor \(F: \A \to \A\).
In this way one obtains the theory of \(F\)-algebras (from generalization of pre-fixed points) and \(F\)-coalgebras (from generalization of post-fixed points); the elements \(\overline{a}\) and \(\overline{b}\) of the above lemmas become, respectively, the initial algebra of \(F\) and the terminal coalgebra of \(F\).
The study of initial algebras and terminal coalgebras allows one to generalize the concept of (co)recursion and (co)induction to categories other than \(\Set\) and has many applications, for instance, in computer science.\\

In Chapter \ref{ch:1} we recall some notions of category theory that we will use through the thesis, and establish some notation.
It can be skipped if the reader is familiar with (co)limits.

Chapter \ref{ch:2} is dedicated to the generalization of pre-fixed points to \(F\)-algebras.
Many examples are presented (with particular attention to the case when \(F\) is a polynomial functor) and {initial algebras} are defined.
Key results of the chapter are Lambeck's Lemma \ref{lemma:lambeck}, which generalizes Lemma 1 above, the Primitive Recursion Theorem \ref{teo:primitive_recursion} and the Induction Principle \ref{th:induction_principle} which show that initial algebras capture and generalize the concepts of recursion and induction.

Questions regarding the existence of an initial algebra are tackled in Chapters \ref{ch:2} and \ref{ch:initial_algebras_iteration}.
The first sufficient condition that we shall see is that of Adámek's Theorem \ref{teo:adamek}: under appropriate conditions the initial algebra of \(F\) is obtained as the colimit of a diagram, the initial-algebra \(\omega\)-chain, constructed by countable iteration of \(F\).
Iteration can be pushed into transfinite territory and yields a stronger sufficient condition (Theorem \ref{teo:adamek_transfinite}).
Moreover, in the case of categories ``sufficiently like \(\Set\)'', many implications can be reversed and we obtain the Initial Algebra Theorem \ref{teo:initial-algebra-theorem} which gives many equivalent conditions for the existence of an initial algebra.

Post-fixed points are generalized to \(F\)-coalgebras in Chapter \ref{ch:5}.
We study with care the dual concept of initial algebras: terminal coalgebras.
Many results can be straightforwardly dualized from the algebraic case such as Lambeck's Lemma \ref{lemma:colambeck} (which generalizes Lemma 2 above) and the Primitive Corecursion Theorem \ref{th:primitive_corecursion}; others, such as the Coinduction Principle \ref{teo:coinduction_principle}, are presented independently from their algebraic version.

In Chapter \ref{ch:6} we turn to iteration for coalgebras.
As before the main results that hold for algebras can be dualized, but the resulting theorems (Theorems \ref{teo:coadamek} and Corollary \ref{coroll:coadamek_transfinite}) have hypothesis which everyday functors are less likely to satisfy than their algebraic counterparts.
We shall see (Remark \ref{rem:tcoalgebra-implies-ialgebra}) that, even in the case of \(\Set\), there is no coalgebraic version of the Initial Algebra Theorem.
Sufficient conditions for the existence of a terminal coalgebra exist in the case of endofunctors over \(\Set\), but they require some set-theoretical machinery (Theorem \ref{teo:accessible-implies-terminal-coalgebra}) or even the Generalized Continuum Hypothesis (Theorem \ref{teo:GCH}).
...
\begin{definition}
  Given a functor \(F \colon \mathscr{D} \to \A\) a \textbf{cone} on \(F\) is an object \(C \in \A\) and a family of arrows \(\left(\pi_D \colon C \to FD\right)_{D \in \mathscr{D}}\) such that for every arrow \(d \colon D_1 \to D_2\) of \(\mathscr{D}\) we have
  \begin{equation}
    \label{eq:cone}
...
\begin{definition}
  Given a functor \(F \colon \mathscr{D} \to \A\) a \textbf{limit} of \(F\) is a cone \((L, (\pi_D)_{D \in \mathscr{D}})\) such that for every other cone \((C, (\rho_D)_{D \in \mathscr{D}})\) there is a unique arrow \(m \colon C \to L\) such that
  \begin{equation*}
...
\begin{definition}
  Given a functor \(F \colon \mathscr{D} \to \A\) a \textbf{cocone} on \(F\) is an object \(C \in \A\) and a family of arrows \(\left(\sigma_D \colon FD \to C \right)_{D \in \mathscr{D}}\) such that for every arrow \(d \colon D_1 \to D_2\) of \(\mathscr{D}\) we have
  \begin{equation}
    \label{eq:cocone}
...
\begin{definition}
  Given a functor \(F \colon \mathscr{D} \to \A\) a \textbf{colimit} of \(F\) is a cocone \((L, (\sigma_D)_{D \in \mathscr{D}})\) such that for every other cocone \((C, (\tau_D)_{D \in \mathscr{D}})\) there is a unique arrow \(m \colon L \to C\) such that
  \begin{equation*}
...
\begin{definition}
  An \textbf{initial object} for a category \(\A\) is an object \(0 \in \A\) such that for every other object \(A \in \A\) there is exactly one arrow from \(0\) to \(\A\).
  Dually a \textbf{terminal object} for \(\A\) is an object \(1 \in \A\) such that for every other object \(A \in \A\) there is exactly one arrow from \(A\) to \(1\).
\end{definition}
...
\begin{definition}
  A functor \(G \colon \A \to \B\) \textbf{preserve limits} if, for every small category \(\D\) and functor \(F \colon \D \to \A\), when the limit of \(F\) exists the limit of \(G \circ F\) is obtained by applying \(G\) to the limit cone.
  That is: if \((L, (\pi_D)_{D \in \D})\) is the limit of \(F\) then \((GL, (G\pi_D)_{D \in \D})\) is the limit of \(G \circ F\).
  As expected \(G\) is said to preserve colimits when, for every small category \(\D\) and functor \(F \colon \D \to \A\), when the colimit of \(F\) exists then the colimit of \(G \circ F\) is the image through \(G\) of the colimit cocone.
\end{definition}
...
\begin{definition}
  An \textbf{algebra} for \(F\) (or an \textbf{\(F\)-algebra}) is a pair \((A, \alpha)\) where \(A\) is an object of \(\mathscr{A}\) (that we call the \textbf{carrier} or \textbf{base object}) and \(\alpha\) is an arrow of type \(FA \to A\) (that we call the \textbf{structure} of the algebra).
\end{definition}
...
To every signature we associate a \textbf{polynomial functor} \(H_\Sigma \colon \Set \to \Set\) that operates as follows on objects \(X\) and arrows \(f\).
\begin{align*}
...
\begin{definition}
  A \textbf{complete partial order} (or a \textbf{CPO}) is a poset \((A, \sqsubseteq)\) in which all \(\omega\)-chains
  \begin{equation*}
  ...
  A function \(f \colon (A, \sqsubseteq) \to (B, \sqsubseteq)\) is \textbf{continous} if
  \begin{itemize}
...
\begin{example}
  Let \(\mathcal{P}_\textsf{f} \colon \Set \to \Set\) be the finite power-set functor. 
...
\begin{remark}
  ...
  We call this operation \textbf{tree-tupling} (below an example of tree-tupling induced by a 3-ary symbol \(\sigma\)).
...
\begin{definition}
  ...
  A morphism \(f \colon \mu F \to A\) of \(\A\) is \textbf{recursively specified}  if there exists an algebra structure \(\alpha \colon FA \to A\) on \(A\) that makes \(f\) into an algebra homomorphism (actually the unique algebra homomorphism).
\end{definition}
...
\begin{definition}
  The \textbf{initial-algebra \(\omega\)-chain} of an endofunctor \(F\) is the diagram
  \begin{equation}
...
\begin{example}
  In a category with binary coproducts and an initial object 1 the initial algebra for \(FX = X + 1\), if it exists, is called the \textbf{natural numbers object} (or \textbf{NNO})  of the category.
...
\begin{definition}
  A function \(f : M \to M'\) between metric spaces \((M, d_M), (M', d_{M'})\) is \textbf{non-expanding}  if \(d_{M'}(f(x), f(y)) \leq d_M(x,y)\) for all \(x,y \in M\).
  A metric space \((M, d_M)\) is \(1\)-bounded if \(d_M(x, y) \leq 1\) for all \(x,y \in M\).
  We denote by \(\MS\) the category of \(1\)-bounded metric spaces and non-expanding maps .
\end{definition}
...
\begin{definition}
  A category \(\A\) is \textbf{algebraically complete} if every endofunctor on it has an initial algebra.
\end{definition}
...
A \(j\)-chain in some category \(\A\) is a functor \(C : j \to \A \); we denote the elements of the chain by \(C_i\), for \(i \in j\), and its arrows by \(c_{i,i'} : C_i \to C_{i'}\) for \(i \leq i' < j\).
...
\begin{example}
  ...
  Trees with no infinite paths are called \textbf{well-founded}.
...
\begin{definition}
  Fixed an object \(A\) in a category \(\A\) a \textbf{subobject} of \(A\) is represented by a monomorphism \(m: B \rightarrowtail A\).
  ...
  If \(\A\) is such that every object \(A \in \A\) has only a set of subobjects then \(\A\) is called \textbf{well-powered}.
\end{definition}
...
When a given object \(A\) has only a set of \(\mathcal{M}\)-subobjects we write \(\Sub_\mathcal{M}(A, \leq)\) for the poset of \(\mathcal{M}\)-subobjects of \(A\); if \(\mathcal{M}\) is the class of all monomorphisms we also write simply \(\Sub(A, \leq)\).
Notice that we do not require the monomorphisms that witness \(\leq\) to be in \(\mathcal{M}\).

From here onwards let \(\mathcal{M}\) denote a class of monomorphisms (of some generic category \(\A\) or otherwise clear from the context) containing all isomorphisms and closed under composition.

\begin{definition}
  \label{def:smooth-subobjects}
  We say that an object \(A\) has \textbf{smooth} \(\mathcal{M}\)\textbf{-subobjects} if \(\Sub_\mathcal{M}(A, \leq)\) is a chain-complete poset (particularly it is a set) where joins of chains are given by colimits of the corresponding chain of subobjects.
  We say that the class \(\mathcal{M}\) itself is \textbf{smooth} if every object has smooth \(\mathcal{M}\)-subobjects.
...
\begin{definition}
  Given a category and a class \(\mathcal{M}\) of monomorphisms we say that an endofunctor \(F\) \textbf{preserves \(\mathcal{M}\)} if for every \(m \in \mathcal{M}\) we have \(Fm \in \mathcal{M}\).
\end{definition}

\begin{definition}
  An \textbf{\(\mathcal{M}\)-pre-fixed point} of an endofunctor \(F\) is an object \(A\) together with a monomorphism \(FA \rightarrowtail A\) in \(\mathcal{M}\).
\end{definition}
...
\begin{definition}
  A \textbf{coalgebra} for \(F\) (or a \(F\)\textbf{-coalgebra}) is a pair \((A, \alpha)\) where \(A\) is an object of \(\A\) and \(\alpha\) is an arrow of type \(A \to FA\).
\end{definition}
...
\begin{example}
  ...
  Over \(\Set\) coalgebras for \(FX = X + 1\) are called \textbf{systems with termination}.
...
\begin{example}
  ...
  A \textbf{deterministic automata} is composed of a set \(S\) of states, a transition function \(\delta_s : S \to S\) for every \(s \in \Sigma\) and a set \(A \subset S\) of \textbf{accepting states}.
...
\begin{example}
  \label{ex:NDA}
  As above let \(\Sigma\) be a set of inputs. A \textbf{non-deterministic automata}, similarly to a deterministic one, is made of a set of states \(S\) with a subset \(A\) of accepting states but the transition functions have codomain \(\mathcal{P}(S)\) i.e.
  \begin{equation*}
...
\begin{definition}
  A \textbf{terminal coalgebra} for an endofuctor \(F\) is a terminal object in \(\Coalg F\).
...
\begin{definition}
  ...
  A morphism \(f: A \to \nu F\) of \(\A\) is \textbf{corecursively specified} if there is a coalgebra structure \(\alpha : A \to FA\) on \(A\) that makes \(f\) into a morphism of coalgebras (actually \(f\) is unique since \(\nu F\) is terminal).
\end{definition}
...
\begin{definition}
  ...
  A \textbf{bisimulation} between them is a relation
  \begin{equation*}
...
\begin{definition}
  The \textbf{diagonal subobject} \(\Delta_A\) of \(A \times A\) is the one represented by \(\intoprod{\id_A}{\id_A}: A \rightarrowtail A \times A\).
\end{definition}

\begin{remark}
  The diagonal subobject \(\Delta_A\) is always a bisimulation on any coalgebra \((A, \alpha)\).
  A bisibulation not contained in \(\Delta_A\) is a \textbf{proper bisimulation} while one contained in \(\Delta_A\) is \textbf{improper}.
...
\begin{theorem}[Coinduction Principle]
  \label{teo:coinduction_principle}
...
\begin{definition}
  The \textbf{terminal-coalgebra} \(\omega^{\op}\)\textbf{-chain} is the diagram
    \begin{equation}
...
\begin{remark}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  is a cone over \((\Sigma_n \times A_i^n, \id_{\Sigma_n} \times a_i^n)_{i<\omega}\) that has limit \((\Sigma_n \times A^n, \id_{\Sigma_n} \times \alpha_i^n)_{i<\omega}\) because the functor \(GX = \Sigma_n \times X^n\) preserves limits of \(\omega^\op\)-chains (it is easy to check that \((\Sigma_n \times A^n, \id_{\Sigma_n} \times \alpha_i^n)_{i < \omega}\) is the limit of the diagram obtained by applying \(G\) to (\ref{eq:omega-chain-ex}) by using the universal property of \((A, \alpha_i)_{i<\omega}\)).
...
\begin{example}
  ...
  \begin{equation*}
    ...
    \end{cases}.
  \end{equation*}
  Then the limit is the set containing all the elements \(x \in \prod_{n < \omega} (n + 1)\) such that
  \begin{equation*}
    (F^n!)(x_{n+1}) = x_n \quad \text{for every \(n < \omega\)}
  \end{equation*}
  by how limits are computed in \(\Set\) (see \cite[Chapter 2, Section 8]{handbook1}).
...
\begin{remark}
  ...
  As an example the connecting function \(H^2_\Sigma(1) \to H_\Sigma(1)\) behaves as following (on the depicted tree)
  \begin{center}
    \begin{tikzpicture}
        \node (s2) at (0, 0){\(\sigma_2\)};
        \node (s1) at (1, 0){\(\sigma_1\)};
        \node (s3) at (2, 0){\(\sigma_3\)};
        \node (root) at (1, 1){\(\sigma_3\)};
        \node (arrow) at (3, 0){\(\mapsto\)};

        \node (bb1) at (4, 0){\(\bot\)};
        \node (bb2) at (5, 0){\(\bot\)};
        \node (bb3) at (6, 0){\(\bot\)};
        \node (r2) at (5, 1){\(\sigma_3\)};
    ...
    \end{tikzpicture}
  \end{center}
  where \(\sigma_k\) is a symbol of \(\Sigma\) of arity \(k\).
\end{remark}
...
\begin{definition}
  ...
  The \textbf{terminal-coalgebra chain} is the \(\Ord^\op\)-chain in \(\A\) with objects \(V_j\) and arrows \(v_{j,i}\) (with \(j \geq i\)) defined by transfinite recursion as follows.
  \begin{align*}
    V_0 = 1 && \text{base case};\\
...
\begin{definition}
  We say that the terminal-coalgebra chain \textbf{converges in} \(\lambda\) \textbf{steps} if \(v_{\lambda + 1, \lambda}\) is an isomorphism.
...
\begin{theorem}
  ...
  \begin{equation*}
    (v_{\lambda + 1, \lambda})^{-1}:V_\lambda \to V_{\lambda+1} = FV_\lambda.
  \end{equation*}
\end{theorem}
...
\begin{definition}
  The \textbf{cofinality} of a cardinal \(\lambda\) is the cardinality of the smallest cofinal subset of \(\lambda\).
  If \(\lambda\) is infinite and its cofinality is \(\lambda\) itself then we say that \(\lambda\) is a \textbf{regular cardinal}.
\end{definition}
...
\begin{definition}
  ...
  Given a regular cardinal \(\lambda\) we say that a functor \(F\) over \(\Set\) is \textbf{\(\lambda\)-accessible} if for every set \(X\) and \(x \in FX\) there is a subset \(m : M \rightarrowtail X, |M| < \lambda\) such that \(x \in \im (Fm)\).
\end{definition}

\begin{remark}
  \label{rem:true-definition}
  To be more precise Definition \ref{def:accessible} is that of a \textbf{\(\lambda\)-bound} set functor while \(\lambda\)-accessible is usually reserved for functors that preserve \(\lambda\)-filtered colimits (this can be useful for categories other than \(\Set\)).
  However, in the case of set functors, the two conditions are equivalent (see \cite[Proposition B.7.1]{coalgebrabook}).
  We will use ``\(\lambda\)-accessible'' since it is most common in recent literature.
\end{remark}
...
\begin{example}
  ...
  Clearly \(X \in \mathcal{P}(X)\) but, if \(m: M \rightarrowtail X\) is a subset of \(X\) such that \(X \in \im(\mathcal{P}m)\), then we must have \(M = X\) so \(|M| = \lambda\) and thus \(\mathcal{P}\) is not \(\lambda\)-accessible.
...
\begin{example}
  ...
  Moreover one can prove that the coproduct of \(\lambda\)-accessible set functors is itself accessible\footnote{By (the dual of) \cite[Proposition 2.15.1]{handbook1} we have that if \(F\) is the coproduct of some \(\lambda\)-accessible set functors, i.e. \(F = \coprod_i F_i\), then for a set \(X\) we have \(F(X) = \coprod_i F_i(X)\).
  Now from \(x \in F(X)\) follows \(x \in F_i(X)\) for some index \(i\) and we obtain the subset of \(X\) that we need to prove that \(F\) is \(\lambda\)-accessible by using that \(F_i\) is \(\lambda\)-accessible.
  This is not unique to set functors.
  It remains true for general \(\lambda\)-accessible functors over categories other than \(\Set\) (see Remark \ref{rem:true-definition}) and even in the case of colimits other than just coproducts; see \cite[Proposition 2.4.5]{Makkai1989AccessibleC}} so a polynomial functor \(H_\Sigma\) is \(\lambda\)-accessible provided that all arities of the symbols of the signature \(\Sigma\) are less than \(\lambda\).
\end{example}
...
\begin{proof}
  Refer to \cite[Proposition 2.1]{Trnková1969}.
\end{proof}
...
\begin{example}
  ...
  This functor has \(\mathbb{N}\) as fixed point since \(\mathcal{P}_{-\aleph_0}(\mathbb{N})\) is a countable set\footnote{Since \(\mathbb{N}\) does not have uncountable subsets we have \(\mathcal{P}_{-\aleph_0}(\mathbb{N}) = \mathcal{P}_\textsf{f}(\mathbb{N})\) and since \(\mathbb{N}\) is an infinite set \(\mathcal{P}_\textsf{f}(\mathbb{N})\) has the same cardinality of \(\mathbb{N}\).}.
  Then by Theorem \ref{teo:set_lambeck_converse} it has an initial algebra; we shall now see that it does not have a terminal coalgebra.
  ...
  Indeed let \(A\) be an uncountable set, \(B\) the set of all countably infinite subsets of \(A\) and \(C = \mathcal{P}_{-\aleph_0}(A)\); then \(B \cup C = \mathcal{P}(A)\) and
  \begin{equation*}
...
\begin{definition}
  ...
  A set functor \(F\) has a \textbf{fixed point pair} if there is a regular cardinal \(\lambda\) such that \(F\) has a fixed point of cardinality \(\lambda\) and one of cardinality \(\lambda^+\) (the smallest cardinal bigger than \(\lambda\)).
\end{definition}

\begin{remark}
  \label{rem:gch}
  Recall that the \textbf{Generalized Continuum Hypothesis (GCH)} is the statement that, for every infinite cardinal \(\lambda\) one has \(\lambda^+ = 2^\lambda\); here with \(2^\lambda\) we indicate the power-set of \(\lambda\).
...
\begin{proof}
  For the ``only if'' direction notice that when a functor \(F\) has fixed point pair \((\aleph_0, \aleph_1)\) then one can adapt the proof of Theorem \ref{teo:GCH} (we only need \(\lambda^+ = 2^\lambda\) for the \(\lambda\) such that \((\lambda, \lambda^+)\) is a fixed point pair to do so; in this case \(\lambda = \aleph_0\)).
