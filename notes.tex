...
\begin{example}
  ...
  The reader can check for himself that this coalgebra is terminal or derive it from the following example.
\end{example}

\begin{proposition}
  Let \(\Sigma\) be a signature and let \(\nu H_\Sigma\) be the set of all \(\Sigma\)-trees (finite and infinite ones) equipped with the \(H_\Sigma\)-coalgebra structure that sends a tree
  \begin{equation*}
    \begin{tikzpicture}
      \node (t1) at (0,-0.5){\(t_0\)};
    ...
    \end{tikzpicture}
  \end{equation*}
  to the element \((\sigma, (t_0, \ldots, t_{n-1}))\) of \(H_\Sigma(\nu H_\Sigma)\).
  This is the terminal coalgebra of \(H_\Sigma\).
\end{proposition}
...
Lambeck's Lemma \ref{lemma:lambeck} dualizes as well.

\begin{lemma}
  A terminal algebra for \(F\) is always a fixed point.
\end{lemma}

\begin{remark}
  As for algebras the power-set functor \(\mathcal{P}\) has no terminal coalgebra because it has no fixed point.
\end{remark}

\begin{remark}
  A converse like Theorem \ref{teo:initial-algebra-theorem} for the case of coalgebras does not exists.
  [Scan the paper given on the other book]
\end{remark}
