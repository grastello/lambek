...
\begin{remark}
  ...
  However, for set functors, the two conditions are equivalent and we will thus use ``\(\lambda\)-accessible'' since it is most common in recent literature.
\end{remark}
...
\begin{example}
  ...
  Moreover one can prove that the coproduct of accessible functors is itself accessible\footnote{\color{teal} Il libro non offre alcuna referenza esplicita ma, se \(F = \coprod_i F_i\), usando la Proposizione 2.15.1 (dualizzata) del Borceaux (volume 1) otteniamo che \(F\) si computa puntualmente cioè \(F(X)\) è il coprodotto degli \(\{F_i(X)\}_i\). A questo punto se \(x \in F(X)\) so che \(x \in F_i(X)\) per qualche \(i\) e ottengo il sottoinsieme che mi serve usando che \(F_i\) è \(\lambda\)-accessibile. Lo scriviamo esplicitamente o semplicemnte puntiamo il lettore alla proposizione del Borceaux?} so a polynomial functor \(H_\Sigma\) is \(\lambda\)-accessible provided that all arities of the symbols of the signature \(\Sigma\) are less than \(\lambda\).
\end{example}
...
\begin{example}
  ...
  This functor has \(\mathbb{N}\) as fixed point since \(\mathcal{P}_{-\aleph_0}(\mathbb{N})\) is a countable set\footnote{\color{teal} Qui non penso sia necessaria alcuna referenza; semplicemente si nota che \(\mathcal{P}_{-\aleph_0}(\mathbb{N}) = \mathcal{P}_\textsf{f}(\mathbb{N})\) siccome \(\mathbb{N}\) non ha sottoinsiemi non-numerabili e poi \(\mathcal{P}_\textsf{f}(\mathbb{N})\) è numerabile.}.
...
\begin{proof}
  For the ``only if'' direction notice that when \(F\) has fixed point pair \((\aleph_0, \aleph_1)\) then one can repeat the proof of Theorem \ref{teo:GCH} (we only need \(\lambda^+ = 2^\lambda\) for the \(\lambda\) such that \((\lambda, \lambda^+)\) is a fixed point pair to do so; in this case \(\lambda = \aleph_0\)).
