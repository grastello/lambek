...
\begin{remark}
  ...
  We shall prove that in a category \(\A\) with binary coproducts, a terminal object and colimits of \(\omega\)-chains, the functor \(FXX = X + 1\) preserves colimits of \(\omega\)-chains.\\
  ...
  By composition with the coprojections of \(A_i + 1\) we obtain that
  \begin{itemize}
...
\begin{example}
  ...
  Explicitly a NNO is a tuple \((N, \iota:N+1 \to N)\) such that for every other \((A, \alpha = [\alpha_0, \alpha_1]:A+1 \to A)\) there is a unique \(h:N\to A\) such that
  \begin{equation*}
    \begin{tikzcd}[sep = large]
...
\begin{example}
  ...
  \begin{tcolorbox}
    DIAGRAM HERE
  \end{tcolorbox}
  So the \(n\)-th element of the \(\omega\)-chain can be thought of as the set \(n = \{0, \ldots, n-1\}\) with the natural order upon it while the arrows are inclusions.
...
\begin{example}
  ...
  Its initial-algebra \(\omega\)-chain in the following sequence
  \begin{tcolorbox}
    diagram
  \end{tcolorbox}
...
\begin{example}
  ...
  If we denote the pair \((x, y) \in X \times X\) by the tree [TREE] we have that
  \begin{tcolorbox}
    DIAGRAM
  \end{tcolorbox}
  ...
  Now fix \(0 < k < \omega\) and consider a tree [TREE \(t_1\), \(t_2\)] in \(F^k0\) that we also regard as the pair \((t_1, t_2)\) for \(t_1, t_2 \in F^{k-1}0\).
  We have
  \begin{align*}
    c_k([TREE t_1, t_2]) &= [TREE t_1, t_2] \in \mu F\\
...
\begin{tcolorbox}
  Adamek's theorem as a special case.
\end{tcolorbox}

We now give an example of a functor that has initial-algebra convergent in more than \(\omega\) steps.
Such functor thus has an initial-algebra not obtainable via Adamek's Theorem. 
...
\begin{example}
  ...
  Conside now the functor \(FX = X^\omega + 1\).
