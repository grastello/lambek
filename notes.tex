...
The goal of this section is to dualize to coalgebras what has been done in Section \ref{ch:initial_algebras_iteration}.
...
\begin{remark}
  ...
  \begin{align*}
    \alpha_0 &=\ ! \\
    \alpha_{n+1} &= A \xrightarrow{\alpha} FA \xrightarrow{F\alpha_n}.
  \end{align*}
...
\begin{theorem}
  ...
  Then \(F\) has a terminal coalbegra \(\nu F = \lim_{n < \omega} F^n1\).
...
\begin{remark}
  ...
  In order to do so we need to prove that for any cone
  \begin{equation*}
    \begin{tikzcd}[sep = large]
  ...
  Finally we can define \(f: B \to H_\Sigma A\) by
  \begin{equation*}
...
\begin{example}
  ...
  \begin{equation*}
    ...
    \end{cases}.
  \end{equation*}
  Then the limit is the set composed of all the elements \(x \in \prod_{n < \omega} (n + 1)\) such that \(f_n(x_{n+1}) = x_n\) by how limits are computed in \(\Set\) (see \cite[Chapter 2, Section 8]{handbook1}).
...
\begin{example}
  ...
  \begin{equation*}
    M^\omega \cong M^n \times \prod_{i \geq n}M.
  \end{equation*}
...
To achieve this goal we begin by giving a [BLA BLA BLA]
...
\begin{remark}
  ...
  \end{equation*}
  In order to show this we begin by identifying \(1 = \{*\} = H^0_\Sigma(1)\) with the \(\Sigma_\bot\) tree formed by a single node (the root) labeled by \(\bot\).
...
\begin{proof}[Alternative proof of Proposition \ref{prop:trees_coalgebras}]
  ...
  \end{equation*}
  Tree-tupling \(\overline{\tau}: H_\Sigma T_\Sigma \to H_\Sigma\) satisfies \(l_{m + 1} \circ \overline{\tau} = H_\Sigma l_m\) because cutting a tree-tupling at the \((m + 1)\)-th layer and re-labeling the leaves (this is \(l_{m +1} \circ \overline{\tau}\)) is the same as cutting and re-labeling maximal subtress at their \(m\)-th layer (this is \(H_\Sigma l_m\)).
...
However this condition isn't satisfied by many everyday functors (e.g. the finite power-set functor \(\mathcal{P}_\textsf{f}\) does not preserve limits of \(\omega^\op\)-chains but, as we shall see [??], it has a terminal coalgeebra).
...
\begin{example}
  ...
  An example of a finitary set functor is \(\mathcal{P}_{\textsf{f}}\), the finite power-set function.
  ...
  It is clear that \(x \in \im(\mathcal{P}_{\textsf{f}}m)\) so we have that \(\mathcal{P}_{\textsf{f}}\) is a finitary functor.
\end{example}
...
\begin{example}
  ...
  \end{equation*}
  with the obvious action on arrows is \(\lambda\)-accessible.
...
\begin{proof}
  ...
  \item[(1)]
    ...
    \end{equation*}
    of \(X\) such that \(|U_x|, |U_y| < \lambda\) and \(x \in \im Fu_x, y \in \im Fu_y\).
    If \(u : U \rightarrowtail V_\lambda\) is the union of \(U_x, U_y\) then we have \(|U| < \lambda\) and \(x, y \in \im Fu\).
    Moreover \((v_{\lambda, i})_{i < \lambda}\) is a limit cone so, by the universal property, every pair of distinct elements of the vertex \(a,b \in V_\lambda\) (and particularly if \(a,b \in U\)) remain distinct under \(v_{\lambda, i}\) for at least one \(i < \lambda\); this is true for every kind of limit cone, but for ones over \(\omega^\op\) it is also the case that \(a, b\) remain distinct under every \(v_{\lambda, j}\) for \(i \leq j < \lambda\). Now let us chose for every pair \((a, b)\) of distinct elements of \(U\) an ordinal \(i_{(a, b)} < \lambda\) such that \(v_{\lambda, i_{(a, b)}}\) keeps \(a, b\) distinct; the set of such ordinals has cardinality at most \(|U \times U| < \lambda\) and thus can't be cofinal because \(\lambda\) is a regular cardinal. This means that there is a \(i_0 < \lambda\) such that every pair of distinct elements of \(U\) remains distinct under \(v_{\lambda, i_0}\) i.e. \(v_{\lambda, i_0} \circ u\) is a monomorphisms.

    Now, since \(U \neq \emptyset\), \(v_{\lambda, i_0} \circ u\) splits and we have that \(Fv_{\lambda, i_0} \circ Fu\) is a monomorphisms as well thus, as \(x, y \in \im Fu\), \(x\) and \(y\) are kept distinct by \(Fv_{\lambda, i_0}\).
    Finally
    \begin{equation*}
  ...
  \item[(2)]
    ...
    This because the chains \((V_i)_{i < \lambda + \lambda}\) and \((FV_i)_{i < \lambda + \lambda}\) have the same limit by the dual of Remark \ref{lemma:final_functors}.
    By the same remark we can disregard the first \(\lambda\) members of this chain and write
    \begin{equation*}
      V_{\lambda + \lambda} = \lim_{i < \lambda} V_{\lambda + i}
    \end{equation*}
    and so by (1) the connecting morphisms as well as the ones forming the cone are all mono.
    ...
    We can assume that \(u \cap v_\lambda\) is non-empty since we can take any element of \(V_{\lambda + \lambda}\) and add it to \(U\) without changing its cardinality.
    ...
    Now from \(u_i = u \cap v_i\) being non-empty, \(x \in \im(Fu)\), \(x \in \im(Fv_i)\) (which we have from definition of \(U\) and hypothesis on \(x\)) and Lemma \ref{lemma:preserve-finite-intersections} we obtain that \(x\in\im(Fu_i)\) for all \(i < \lambda\).
    ...
    For that ordinal \(i_0\) we have \(u_{i_0} = u_\lambda = u \cap v_\lambda \subseteq v_{\lambda}\) so \(\im(Fu_0) \subseteq \im(Fv_{\lambda})\) and this gives that \(x \in \im(Fv_{\lambda})\).
  \end{itemize}
\end{proof}
...
\begin{example}
  ...
  Note that \(\mathcal{P}_\lambda\) is a subfuctor of \(\mathcal{P}_{-\aleph_0}\) so Proposition \ref{prop:subfunctor} applies: if \(\nu \mathcal{P}_{-\aleph_0}\) exists then it is at least as large as \(\nu \mathcal{P}_{\textsf{f}}\) which is uncountable (prove it?).
...
The previous example shows, as already observed, that there is no Theorem \ref{} for \(\Set\).
There is, however, another sufficient condition [BLA BLA BLA].
...
For our sufficient condition we will need to assume the Generalized Continuum Hypothesis which is independent from ZFC.
...
\begin{remark}
  ...
  Under this hypothesis if \(k < \lambda\) and \(\lambda\) is a regular cardinal then \(\lambda^k = \lambda\) (see \cite[Theorem 5.15 (iii)]{jech}).
  Moreover we recall that we have \(\lambda^\lambda = 2^\lambda\) for every infinite cardinal regardless of GCH.
\end{remark}
...
\begin{lemma}
  ...
  Then for every \(x \in FX\) there is a subset \(y : Y \rightarrowtail X\) such that \(|Y| < |X|\) and \(x \in \im(Fm)\).
\end{lemma}
...
\begin{proof}
  ...
  \item[(1)]
    ...
    \begin{itemize}
    \item[\(\bullet\)] The base case in trivial.
    ...
    \item[\(\bullet\)] For the limit step let \(j < \lambda\) be a limit ordinal and assume \(|V_i| \leq \lambda\) for every \(i < \lambda\).
      ...
      But \(\lambda^\kappa = \lambda\) by Remark \ref{rem:gch}.
    \end{itemize}
  \item[(2)]
  ...
  \item[(3)]
    ...
    Now let \(x_0 \in FM_0\) be such that \(Fm_0(x_0) = x\) and notice that \(|M_0| < \lambda^+\) implies \(|M_0| \leq \lambda\); we can now apply Lemma \ref{lemma:pseudo-accessability} to the fixed point \(Z = \lambda\) and the set \(M_0\) to obtain a subset \(m_1: M_1 \rightarrowtail M_0\) with \(x_0 \in \im(Fm_1)\) and \(|M_1| < \lambda\).
...
\begin{proof}
  ...
  \end{equation*}
  on objects and naturally on arrows.
  Now if \(X\) has cardinality \(\aleph_0\) or \(\aleph_1\) then \(\mathcal{-\aleph_0, \aleph_1}(X) = \mathcal{P}_{\textsf{f}}(X)\) which gives that both \(\aleph_0\) and \(\aleph_1\) are fixed points; hence \(\mathcal{P}_{-\aleph_0, \aleph_1}\) has a fixed point pair.
