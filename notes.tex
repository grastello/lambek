...
\begin{definition}
  ...
  \begin{center}
    \begin{tikzcd}[sep=large]
      A \ar[r, "\alpha"] \ar[d, "Ff"] & FA \ar[d, "f"] \\
      B \ar[r, "\beta"] & FB
    \end{tikzcd}
  \end{center}
  commutes.
...
\begin{example}
  \label{ex:NDA}
  As above let \(\Sigma\) be a set of inputs. A \textbf{non-deterministic automata} \marginnote{non-deterministic automata}, like a deterministic one, is made of a set of states \(S\) with a subset \(A\) of accepting states but the transition functions have codomain \(\mathcal{P}(S)\) i.e.
  \begin{equation*}
...
\begin{example}
  ...
  \end{equation*}
  of outputs; this is an element of \(\Sigma^\omega\), the set of infinite sequences (also called \textbf{streams}\marginnote{streams}) over \(\Sigma\).
...
\begin{example}
  ...
  We know from Example \ref{ex:transition_systems_with_outputs} that the terminal coalgebra for \(F\) is \(\Omega^\infty\), the set of \(\Sigma\)-streams, with structure given by \(\tau = \intoprod{\head}{\tail}\).
  ...
  \begin{equation*}
    \intoprod{\head}{\tail} \circ h = \alpha \circ (\id \times h)
  \end{equation*}
  ...
  \end{equation*}
  So alternating streams are also sort of corecursively specifiable.
...
We are now left with dualizing the Induction Principle (Theorem \ref{th:induction_principle}).
However, instead of doing it directly, we first introduce the concept of a bisimulation (WHY?).
...
Some examples should clarify the matter.

\begin{example}
  Consider a pair of systems with termination (see Example ??)
  \begin{equation*}
